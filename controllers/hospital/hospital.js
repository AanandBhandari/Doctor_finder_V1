...
exports.setOPD = async (req, res) => {
  ...
  if (actualStartTime < now) {
    return res.status(400).json({ error: "Start date and time should be from at least today and a hour later from now respectively" })
  }
  ...
  async function Init0(newOPD) {
    ...
    const getMillis = new Date(enddate).getTime() - new Date(startdate).getTime()//need to check date without creting new instance
    ...
    for (let i = 0; i < noofDays; i++) {
      const availabletimeslot = new Array(noOfTimeInterval).fill(0)
